        ...
        <div class="sz_bigbox">
            <div class="left_box">
                ...
                <div style="padding:20px 50px 20px 50px;height: auto">
                    <h1 style="text-align: center;margin-top: 30px">查看【${goods.name}】商品</h1>
                    ...
                    <ul class="test_ul">
                        <li style="margin-left: 12px" ><div id="img" class="photo" style="border: 1px solid slategrey;width: 240px;height: 180px;padding: 2px">
                            ${goods.photo}
                        </div></li>
                    ...
                    <div style="margin-top: 20px;border: 1px solid slategrey;padding: 10px;font-size: 18px;width: 100%" class="mini-panel"
                        ...
                        <div style="margin-left: 80px">
                            ...
                            <tr><label ><b>卖家联系方式：</b></label><br></tr>
                             ...
                             <label ><b>手机号：</b></label><span class="f_b f_orange" >${goodsuserinfo.moblie}</span></tr>
                        </div>
                    ...
                    <div style="margin-top: 25px;border: 1px solid slategrey;padding: 10px;font-size: 15px;width: 100%" class="mini-panel"
                        ...
                        <div style="margin-left: 80px">
                            <tr> <span class="f_b f_orange" >${goods.miaoshu}</span>
                            </tr><br>
                        </div>
                    </div>

                    <div style="margin-top: 25px;border: 1px solid slategrey;padding: 10px;font-size: 15px;width: 100%" class="mini-panel"
                         showCollapseButton="true"  collapseOnTitleClick="true" title="商品评价等级">
                        <div style="background:#FAFAFA;padding:10px 20px 10px 20px;line-height:24px;">
                        </div >
                        <hr style="border:none;border-top:4px groove skyblue;" /><br>

                        <div style="margin-left: 80px;background: #E8E8E8 " id="sppj">
                            <ul class="test_ul">
                           <li> <p class="all">
                                <input type="radio" name="b" value="0" checked="checked" disabled="true"　readOnly="true"/>
                                <span>★</span>
                                <input type="radio" name="b" value="1" <c:if test="${level eq 1}">checked="checked"</c:if> disabled="true"　readOnly="true"/>
                                <span>★</span>
                                <input type="radio" name="b" value="2" <c:if test="${level eq 2}">checked="checked"</c:if> disabled="true"　readOnly="true" />
                                <span>★</span>
                                <input type="radio" name="b" value="3" <c:if test="${level eq 3}">checked="checked"</c:if> disabled="true"　readOnly="true"/>
                                <span>★</span>
                                <input type="radio" name="b" value="4" <c:if test="${level eq 4}">checked="checked"</c:if> disabled="true"　readOnly="true"/>
                                <span>★</span>
                                <input type="radio" name="b" value="5" <c:if test="${level eq 5}">checked="checked"</c:if> disabled="true"　readOnly="true"/>
                                <span>★</span>
                            </p></li>
                              <li style="margin-left: 680px;">
                                  <ul class="test_ul">
                                      <li> <span style="line-height:30px;font-size: 20px ">评分:&nbsp;&nbsp;</span></li>
                                      <li>
                                          <input class="mini-textbox-border" style="width: 30px;height: 30px;background: #F3F3F3;color: #FF6600;font-size: 20px"
                                                 value="${score}" disabled="true"　readOnly="true">
                                      </li>
                                      <li><span style="line-height: 30px;font-size: 20px">/5</span></li>
                                  </ul>
                    ...
                    <div style="margin-top: 25px;border: 1px solid slategrey;padding: 10px;font-size: 15px;width: 100%" class="mini-panel"
                         showCollapseButton="true"  collapseOnTitleClick="true" title="用户评价">
                        <div style="background:#FAFAFA;padding:10px 20px 10px 20px;line-height:24px;">

                        </div >
                        <hr style="border:none;border-top:4px groove skyblue;" /><br>

                        <div style="margin-left: 80px;background: #E8E8E8 ">

                            <c:forEach items="${evaluate}" var="evaluate" varStatus="status">
                                <div id="${status.index+1}">
                                <hr style="height:1px;border:none;border-top:1px dashed #0066CC;" />
                                <tr>#${status.index+1} &nbsp;&nbsp;&nbsp;&nbsp;<span class="f_b f_orange" ><b>${evaluate.username}</b>:</span>
                                </tr> <span style="color: #7b7b7b;margin-left: 500px">${evaluate.postdateStr}</span><br>
                                <tr> <span class="spanl"  style="color: #1a67e2;margin-left: 50px" ><b>${evaluate.evaluate}</b></span>
                                </tr><br>
                                </div>
                            </c:forEach>
                            <c:if test="${evaluate eq null}">暂无用户评论和商品评价等级</c:if>
                        </div>
                    </div>
                ...
                <tr>
                   <c:if test="${goodsuserinfo.name != userinfo.name && admininfo eq null}">

                       <td colspan="2" ><button style="margin-left: 40%;margin-bottom: 10px;" class="c_bt" onclick="newRow()">加入购物车</button><button style="margin-left: 30px;margin-bottom: 10px;" class="c_bt" onclick="bought()">立即购买</button></td>
                   </c:if>
                </tr>
            </div>
        </div>
